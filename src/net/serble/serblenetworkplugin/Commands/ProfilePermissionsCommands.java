...
public class ProfilePermissionsCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(Functions.translate("&cUsage: /profileperms <set/unset> <profile uuid> <permission node> <true/false>"));
            return true;
        }

        if (args[0].equalsIgnoreCase("set")) {
            if (args.length != 4) {
                sender.sendMessage(Functions.translate("&cUsage: /profileperms set <profile uuid> <permission node> <true/false>"));
                return true;
            }
