...
public class ProfileCommand implements CommandExecutor, TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2) for (String profile : PlayerUuidCacheHandler.getInstance().getProfileList(p.getUniqueId())) {
            if (!profile.toLowerCase().startsWith(args[0].toLowerCase())) continue;
            results.add(profile);
        }
